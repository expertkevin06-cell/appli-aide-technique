/* access.js v2 — Accès tiers Firebase : autorisation, révocation, RÉVOCATION FORCÉE fiable,
   bouton 🔐 admin partout, auto-vérifications. Config déjà renseignée. */
(function(){
'use strict';
...
/* VÉRIFICATION : consulte TOUJOURS Firestore si en ligne (révocation forcée immédiate) */
...
/* GARDE : re-vérifie toutes les 30 s même app ouverte → révocation forcée appliquée en direct */
...
window.renderAccessAdmin=async function(){
 ...
 if(!ENABLED){box.innerHTML='<p class="muted"><b>Contrôle d\u2019accès tiers désactivé</b> (Firebase non configuré) : l\u2019app reste ouverte à tous. La config est déjà pré-remplie dans ce fichier ; vérifiez qu\u2019elle est bien commitée sur GitHub.</p>';return;}
 try{
  const db=await loadFB();
